...
DbgSST15::DbgSST15(SST::ComponentId_t id, const SST::Params &params)
  ...
  output.init("DbgSST15[" + getName() + ":@p:@t]: ", Verbosity, 0,
              SST::Output::STDOUT);
  ...
  if (traceMode & 1)
    output.verbose(CALL_INFO, 1, 0, "tracing SEND events\n");
  if (traceMode & 2)
    output.verbose(CALL_INFO, 1, 0, "tracing RECV events\n");
  if (traceMode > 2)
  ...
  // setup the rng
  mersenne =
      new SST::RNG::MersenneRNG(params.find<unsigned int>("rngSeed", 1223));
  mersenne2 =
      new SST::RNG::MersenneRNG(params.find<unsigned int>("rngSeed", 1224));

  // setup the links
  for (unsigned i = 0; i < numPorts; i++) {
    linkHandlers.push_back(configureLink(
        "port" + std::to_string(i),
        new SST::Event::Handler2<DbgSST15, &DbgSST15::handleEvent>(this)));
  }
#if PROBE
  ...
  int probeBufferSize =
      params.find<int>("probeBufferSize", DEFAULT_PROBE_BUFFER_SIZE);
  ...
  probe_ = std::make_unique<DbgSST15_Probe>(
      this, &output, probeMode, probeStartCycle, probeEndCycle, probeBufferSize,
  ...
  std::cout << "Runtime type test_myPB: " << typeid(test_myPB).name()
            << std::endl;
  std::cout << "Runtime type *test_myPB: " << typeid(*test_myPB).name()
            << std::endl;
  std::cout << "Runtime type test_ProbeBuffer: "
            << typeid(test_ProbeBuffer).name() << std::endl;
  std::cout << "Runtime type *test_ProbeBuffer: "
            << typeid(*test_ProbeBuffer).name() << std::endl;
...
void DbgSST15::tickleBits() {
  tickle_counter++;
  if (tickle_counter % 2 == 0)
    v_bitset42.flip();
  if (tickle_counter % 3 == 0)
    v_vecbool.flip();
  std::stringstream s;
  s << "S" << tickle_counter;
  ...
  if (tickle_counter % 7 == 0)
    v_tuple_u32_dbl_str = {tickle_counter, 1.0 / (double)tickle_counter,
                           s.str()};
}
...
#if PROBE
DbgSST15_Probe::DbgSST15_Probe(SST::Component *comp, SST::Output *out, int mode,
                               SST::SimTime_t startCycle,
                               SST::SimTime_t endCycle, int bufferSize,
                               int port, int postDelay, uint64_t cliControl)
    : ProbeControl(comp, out, mode, startCycle, endCycle, bufferSize, port,
                   postDelay, cliControl) {
  probeBuffer = std::make_shared<ProbeBuffer<event_atts_t>>(bufferSize);
  setBufferControls(probeBuffer);
}

void DbgSST15_Probe::capture_event_atts(uint64_t cycle, uint64_t sz,
                                        DbgSST15Event *ev) {
  if (!sampling())
    return;
...
void DbgSST15_Probe::serialize_order(
