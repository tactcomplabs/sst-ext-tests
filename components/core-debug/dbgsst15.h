...
ImplementSerializable(SSTDEBUG::DbgSST15::DbgSST15)
  ...
  std::vector<bool> v_vecbool = {true,  false, true, true,
                                 false, false, true, true};
  // pair and tuple (sst-simulator/sst-core PR#1487)
  std::pair<uint64_t, std::string> v_pair_u64_str = {42, "forty-two"};
  std::tuple<uint32_t, double, std::string> v_tuple_u32_dbl_str = {8, 1.0 / 8.0,
                                                                   "eight"};
...
public:
  DbgSST15_Probe(SST::Component *comp, SST::Output *out, int mode,
  ...
  struct event_atts_t {
    ...
    friend std::ostream &operator<<(std::ostream &os, const event_atts_t &e) {
      os << std::dec << "cycle=" << e.cycle_ << " sz=" << e.sz_
         << " deliveryTime=" << e.deliveryTime_ << " priority=" << e.priority_
         << " orderTag=" << e.orderTag_ << " queueOrder=" << e.queueOrder_;
      return os;
    }
