...
ProbeControl::ProbeControl(SST::Component *comp, SST::Output *out, int mode,
                           SST::SimTime_t startCycle, SST::SimTime_t endCycle,
                           int bufferSize, int port, int postDelay,
                           uint64_t cliControl)
    : comp_(comp), out_(out), probeBufCtl_(nullptr), mode_(mode),
      startCycle_(startCycle), endCycle_(endCycle), bufferSize_(bufferSize),
      port_(port), postDelayCounter_(postDelay), cliControl_(cliControl) {
...
void ProbeControl::updateSyncState(SST::SimTime_t cycle) {
  syncCycle = cycle;
  switch (syncState_) {
  case SyncState::WAIT:
...
void ProbeControl::updateProbeState(SST::SimTime_t cycle) {
...
void ProbeControl::handleSyncPointActions() {
  ...
  if (syncActions_.f.shutdown) {
    ...
    probeBufCtl_->reset_buffer();
    postDelayCounter_ =
        postDelayInitCount_; // TODO move counter into bufferControl?
    probeState_ = ProbeState::PRE_SAMPLING;
  } else {
...
void ProbeControl::sample() {
  ...
  if (cliControl_.f.probe_samples)
    do_cli = true;
  else if (cliControl_.f.probe_trig2samples &&
           (probeState_ == ProbeState::POST_SAMPLING))
    do_cli = true;
  if (do_cli)
    updateCLI();
...
ProbeBufCtl::ProbeBufCtl(size_t sz) : sz_(sz) { tags.resize(sz); };
...
void ProbeBufCtl::reset_trigger() { state = CLEAR; }
...
ProbeSocket::ProbeSocket(uint16_t port, ProbeControl *probeControl,
                         SST::Component *comp, SST::Output *out)
    ...
    case CMD::RUN:
      if (args.size() > 1) {
        try {
          ...
          std::cout << "could not set number of events. " << e.what()
                    << std::endl;
          response << "?";
        }
      } else {
...
void ProbeSocket::splitStr(std::string s, const char *delim,
...
void ProbeSocket::joinStr(size_t startpos, std::vector<std::string> v,
                          const char *delim, std::string &s) {
  s.clear();
  size_t n = v.size();
