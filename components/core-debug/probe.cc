...
ProbeControl::ProbeControl(SST::Component* comp, SST::Output* out, int mode, SST::SimTime_t startCycle,
    ...
    cliControl_(cliControl)
{
    ...
    if ( !useDelayCounter_ ) out_->verbose(CALL_INFO, 1, 0, "continue post-trigger sampling\n");

    if ( mode_ != 1 ) out_->fatal(CALL_INFO, -1, "probeMode>1 is not supported\n");
...
ProbeControl::updateProbeState(SST::SimTime_t cycle)
{
    ...
    if ( cliControl_.f.probe_states && (lastProbeState_ != probeState_) ) updateCLI();
...
ProbeControl::handleSyncPointActions()
{
    ...
    if ( (!useDelayCounter_) && (probeState_ == ProbeState::POST_SAMPLING) ) probeState_ = ProbeState::WAIT;

    // Only when probe is in WAIT state is a checkpoint action needed
    if ( probeState_ != ProbeState::WAIT ) return;
...
ProbeControl::sample()
{
    if ( useDelayCounter_ && (probeState_ == ProbeState::POST_SAMPLING) ) postDelayCounter_--;
...
{
    if ( probeState_ != ProbeState::PRE_SAMPLING ) return;
    if ( cond ) {
...
ProbeBufCtl::ProbeBufCtl(size_t sz) :
    sz_(sz)
{
    tags.resize(sz);
};
...
void
ProbeBufCtl::reset_trigger()
{
    state = CLEAR;
}

void
ProbeBufCtl::markAsTriggerRec()
{
    state = TRIGREC;
}
...
ProbeBufCtl::capture()
{
    // child class calls this first to establish pointer state before saving
    // record When this buffer fills it writes over the oldest data
    ...
    // std::cout << "state=" << getTrigStateChar() <<  " sz=" << sz_ << "
    // num_recs=" << num_recs << " first=" << first << " cur=" << cur <<
    // std::endl;
...
void
ProbeBufCtl::render_buffer(std::ostream& os)
{
    ...
    if ( state == OVERRUN ) {
        ...
        os << std::endl;
        if ( samples_lost > 0 ) os << "# records discarded " << samples_lost << std::endl;
    }
...
ProbeSocket::create()
{
    ...
    if ( rc < 0 ) return RESULT::BINDING_ERROR;
...
ProbeSocket::cli_handler()
{
    ...
    if ( --runEventCounter_ > 0 ) return ProbeSocket::RESULT::SUCCESS;
    ...
    do {
        ...
        CMD cmd = CMD::UNKNOWN;
        if ( str2cmd.find(args[0]) != str2cmd.end() ) cmd = str2cmd.at(args[0]);
        switch ( cmd ) {
        case CMD::CLICONTROL:
            if ( args.size() > 1 ) {
                try {
...
{
    if ( str2cmd.find(in) == str2cmd.end() ) return false;
