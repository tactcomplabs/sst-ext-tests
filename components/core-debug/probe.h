...
static const std::map<SyncState, std::string> syncState2Str { { SyncState::INVALID, "invalid" },
    { SyncState::IDLE, "idle" }, { SyncState::WAIT, "wait" }, { SyncState::ACTIVE, "active" } };

/// ProbeState only valid when SyncState is ACTIVE
enum class ProbeState { IDLE, RESTART, PRE_SAMPLING, POST_SAMPLING, WAIT };

static const std::map<ProbeState, std::string> probeState2Str { { ProbeState::IDLE, "idle" },
    { ProbeState::RESTART, "restart" }, { ProbeState::PRE_SAMPLING, "pre-sampling" },
    { ProbeState::POST_SAMPLING, "post-sampling" }, { ProbeState::WAIT, "wait" } };
...
union CLI_CTRL {
    ...
    struct
    {
        ...
        uint64_t chkpt_cli          : 1; // [8] [1] entered CLI from checkpoint, [0] entered
                                         // CLI from component probe
    } f;
...
public:
    ...
    ProbeControl(SST::Component* comp, SST::Output* out, int probeMode, SST::SimTime_t probeStartCycle,
        SST::SimTime_t probeEndCycle, int probeBufferSize, int probePort, int probePostDelay, uint64_t cliControl);
    // skk
    ~ProbeControl();
    ...
    void          trigger(bool cond); // TODO pick a more specific name that differentiates
                                      // this from other triggers
...
private:
    ...
    int            postDelayCounter_;   ///< Post trigger delay (-1 to post-trigger sample
                                        ///< until checkpoint)
    int            postDelayInitCount_; ///< Delay counter initial value
    CLI_CTRL       cliControl_ = 0;     ///< controls for breaking into interactive mode
    bool           useDelayCounter_;    ///< when 0 post-trigger sampling continues until
                                        ///< checkpoint.
...
public:
    ...
    const std::map<TRIGGER_STATE, char> trig2char { { CLEAR, '-' }, { TRIGREC, '!' }, { TRIGGERED, '+' },
        { OVERRUN, 'o' } };
    ...
    // virtual void render(std::ostream& os, size_t idx, char pfx) = 0; // print a
    // rec to ostream
    void   render(std::ostream& os, size_t idx, char pfx); // print a rec to ostream
    // virtual void render_trigger_rec(std::ostream&, char pfx) = 0; // print the
    // saved trigger rec
...
template <typename T>
class ProbeBuffer final : public ProbeBufCtl
{
public:
    ProbeBuffer(size_t sz) :
        ProbeBufCtl(sz)
    {
        buf.resize(sz);
    };
    ...
    void capture(T& rec)
    {
        ...
        assert(cur < sz_);
        buf.at(cur) = rec;
        if ( tags[cur] == TRIGREC ) trigger_rec = rec;
    }
    ...
    void render_trigger_rec(std::ostream& os, char pfx) /* skk override */ { os << pfx << ' ' << trigger_rec; }
...
public:
    ...
    const std::map<const std::string, CMD> str2cmd { { "clicontrol", CMD::CLICONTROL }, { "component", CMD::COMPONENT },
        { "cycle", CMD::CYCLE }, { "disconnect", CMD::DISCONNECT }, { "dump", CMD::DUMP }, { "echo", CMD::ECHO },
        { "help", CMD::HELP }, { "hostname", CMD::HOSTNAME }, { "numrecs", CMD::NUMRECS },
        { "probestate", CMD::PROBESTATE }, { "run", CMD::RUN }, { "spin", CMD::SPIN }, { "syncstate", CMD::SYNCSTATE },
        { "trigstate", CMD::TRIGSTATE }, { "?", CMD::HELP }, { "", CMD::UNKNOWN } };
    const std::map<CMD, const std::string> cmd2help {
        ...
        { CMD::RUN, "run   : continue simulation until the next event defined by "
                    "clicontrols\n"
