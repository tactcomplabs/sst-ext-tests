...
union Actions {
  ...
  struct {
    ...
    uint64_t
        cont : 1; // continue post-trig sampling. reset delay counter if used.
...
union CLI_CTRL {
  ...
  struct {
    ...
    uint64_t
        probe_trig2samples : 1; // [1] Every probe sample from trigger forward
...
public:
  ...
  ProbeControl(SST::Component *comp, SST::Output *out, int probeMode,
               SST::SimTime_t probeStartCycle, SST::SimTime_t probeEndCycle,
  ...
  inline bool triggering() const {
    return probeState_ == ProbeState::PRE_SAMPLING;
  }
  ...
  inline std::string const getSyncStateStr() {
    return syncState2Str.at(syncState_);
  }
  inline std::string const getProbeStateStr() {
    return probeState2Str.at(probeState_);
  }
...
private:
  ...
  ProbeState probeState_ =
      ProbeState::IDLE; ///< State managed by component level probe
  ...
  SST::SimTime_t
      startCycle_; ///< When checkpoint >= probeStartCycle sampling begins
...
protected:
  ...
  int samples_lost =
      0; // number of samples sampled but overwritten in circular buffer
...
public:
  ...
  const std::map<CMD, const std::string> cmd2help{
      ...
      {CMD::DISCONNECT,
       "disconnect from probe and allow simulation to complete"},
  ...
  ProbeSocket(uint16_t port, ProbeControl *probeControl, SST::Component *comp,
              SST::Output *out);
  ~ProbeSocket();
...
private:
  ...
  void joinStr(size_t startpos, std::vector<std::string> v, const char *delim,
               std::string &s);
