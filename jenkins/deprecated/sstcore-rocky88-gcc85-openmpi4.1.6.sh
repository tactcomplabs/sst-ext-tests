...
export TERM=linux
export SST_INSTALL=/jenkins/sst-gcc85-openmpi4.1.6-$BRANCH
export PATH=/pkgs/Linux/Rocky88/python-3.13.7/bin:$PATH:/pkgs/Linux/Rocky88/openmpi-4.1.6/gcc85/bin
rm -Rf $SST_INSTALL/*
if [ "$CLANGFORMAT" = true ]; then
	./scripts/clang-format-test.sh --format-exe /pkgs/Linux/Ubuntu-22.04/LLVM/LLVM-20.1.0-Linux-X64/bin/clang-format
fi
./autogen.sh
if [ "$DEBUG" = true ]; then
...
if [ "$EXTTEST" = true ] ; then
	...
	mkdir build
	cd build
    #KG TODO enable -Werror
    if [ "$VALGRIND" = false ]; then
		cmake -DENABLE_ALL_TESTS=ON -DSST_WERROR=OFF $EXTTESTASAN $EXTTESTARGS ../
    else
    	cmake -DENABLE_ALL_TESTS=ON -DSST_WERROR=OFF -DENABLE_VALGRIND=ON $EXTTESTARGS ../
    fi
