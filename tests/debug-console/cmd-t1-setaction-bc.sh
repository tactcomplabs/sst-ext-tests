...
#EXT_TEST TEST_FILE_DESC "Check that trace set action triggers with 1 thread"
#EXT_TEST TIMEOUT 30
...
setHandler 0 bc
# CHECK 0 printWatchpoint 0\nWP0: TriggerCount 0 : BC : .?c0/xorshift/w CHANGED  : bufsize = 32 postDelay = 4 : .*x .*y .*z  : set c0/xorshift/z 50
printWatchpoint 0
trace w changed : 32 4 : w x y z : printTrace
setHandler 1 bc
# CHECK 1 printWatchpoint 1\nWP1: TriggerCount 0 : BC : .?c0/xorshift/w CHANGED  : bufsize = 32 postDelay = 4 : .*x .*y .*z  : printTrace
printWatchpoint 1
run
EOF
...
if [ $DO_BASELINE -eq 1 ]; then
  echo "baseline only"
  LAUNCH="sst --verbose=$VERBOSE $CONFIG"
else
  LAUNCH="sst --verbose=$VERBOSE --interactive-start=0s $CONFIG"
fi
echo $LAUNCH
