...
for sig in $SIG; do
  for action2 in $ACTION2; do
    ...
    LAUNCH="sst --verbose=$VERBOSE --checkpoint-prefix=$PREFIX --checkpoint-sim-period=100ms --sigalrm=$action2(interval=1s) $CONFIG"
    echo $LAUNCH
    $LAUNCH > $OUTFILE 2>&1
    retVal=$?
    ...
    LAUNCH="sst --verbose=$VERBOSE --load-checkpoint $CKPTDIR"
    echo $LAUNCH
    $LAUNCH > $OUTFILE 2>&1 
    retVal=$?
